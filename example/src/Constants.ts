...
//        "velocity-network-testnet://issue?request_uri=https%3A%2F%2Fstagingagent.velocitycareerlabs.io%2Fapi%2Fholder%2Fv0.6%2Forg%2Fdid%3Aion%3AEiByBvq95tfmhl41DOxJeaa26HjSxAUoz908PITFwMRDNA%2Fissue%2Fget-credential-manifest%3Fid%3D624d65daf18484b8525288c3%26credential_types%3DEmploymentPastV1.1"
  ...
  static getCredentialIdsToRefresh = (environment: VCLEnvironment) => {
    return environment === VCLEnvironment.Dev ? [Constants.CredentialId1Dev, Constants.CredentialId2Dev,] : [Constants.CredentialId1Staging, Constants.CredentialId2Staging,]
  }
  ...
  static readonly getIdentificationList = (environment: VCLEnvironment) => {
    return environment === VCLEnvironment.Dev ? Constants.IdentificationListListDev : Constants.IdentificationListStaging;
  }
  ...
  static readonly VerifiedProfileDescriptorDev: VCLVerifiedProfileDescriptor = {
    did: Constants.IssuerDidDev,
  };
  static readonly VerifiedProfileDescriptorStaging: VCLVerifiedProfileDescriptor = {
    did: Constants.IssuerDidStaging,
  };
  static getVerifiedProfileDescriptor = (environment: VCLEnvironment) => {
    return environment === VCLEnvironment.Dev ? Constants.VerifiedProfileDescriptorDev : Constants.VerifiedProfileDescriptorStaging;
  }
  ...
  private static readonly getServiceBaseUrl = (
  ...
  public static readonly getJwtSignServiceUrl = (
    environment: VCLEnvironment
  ): string => {
    return `${Constants.getServiceBaseUrl(environment)}/api/jwt/sign`;
  };

  public static readonly getJwtVerifyServiceUrl = (
    environment: VCLEnvironment
  ): string => {
    return `${Constants.getServiceBaseUrl(environment)}/api/jwt/verify`;
  };

  public static readonly getCreateDidKeyServiceUrl = (
    environment: VCLEnvironment
  ): string => {
